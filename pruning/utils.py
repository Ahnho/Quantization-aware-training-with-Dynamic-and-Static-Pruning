...
def get_weight_threshold_nlp(model, rate, args):
    importance_all = None
...
def get_weight_threshold(model, rate, args):
    ...
    for name, item in model.module.named_parameters():
        # if len(item.size())==4 and 'mask' not in name:
        # print(name)
        # if ('weight' in name or 'downsample_p.0' in name) and 'mask' not in name:
        if 'weight' in name and ('conv' in name or 'fc' in name)  and 'mask' not in name:
            weights = item.data.view(-1).cpu()
            # print(weights)
            # grads = item.grad.data.view(-1).cpu()
